...
class DataAnalyzer:
    ...
    def missing_by_group_cross(self, features: List[str] = None,
        ...
        # 注意：此处使用线程池而非进程池。
        # 原实现用 use_process=True 会把嵌套闭包 pickle 到子进程，导致
        # "Can't pickle local object" 崩溃；且闭包捕获了整个 data_copy，
        # 序列化开销巨大。groupby/isna 等操作主要在 pandas/numpy 的 C 层执行
        # 并释放 GIL，用线程池即可获得并行收益且不需要可 pickle 的对象。
        rows = self._parallel_feature_map(features, _build_single_feature_pivot, n_jobs, use_process=False)
