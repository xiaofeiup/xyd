...
class DataAnalyzer:
    ...
    def missing_by_group(self, features: List[str] = None,
        ...
        # 只复制必要的列
        required_cols = list(set(features + ([group_col] if group_col else []) + ([date_col] if date_col else [])))
    ...
    def missing_by_group_cross(self, features: List[str] = None,
        ...
        # 一次性处理所有特征，避免重复的groupby操作
        for feature in features:
            if feature not in data_copy.columns:
                continue

            # 创建缺失值指示列（向量化）
            is_missing = data_copy[feature].isna().astype(int)
            
            # 构建临时DataFrame用于groupby
            temp_df = pd.DataFrame({
                '_cat_group': data_copy['_cat_group'],
                '_date_group': data_copy['_date_group'],
            ...
            # 使用groupby计算统计（一次性）
            if value_type == 'count':
                pivot = temp_df.groupby(['_cat_group', '_date_group'])['_is_missing'].sum().unstack(fill_value=0)
            else:  # rate
                # 计算缺失率：缺失数 / 总数
                grouped = temp_df.groupby(['_cat_group', '_date_group']).agg({
                    '_is_missing': 'sum',
                    '_count': 'sum'
                })
                pivot = (grouped['_is_missing'] / grouped['_count']).unstack(fill_value=0)
                pivot = pivot.applymap(lambda x: f"{x:.4f}")
            
            # 添加行总计（向量化）
            if value_type == 'count':
                row_totals = temp_df.groupby('_cat_group')['_is_missing'].sum()
            else:
                row_agg = temp_df.groupby('_cat_group').agg({'_is_missing': 'sum', '_count': 'sum'})
                row_totals = (row_agg['_is_missing'] / row_agg['_count']).apply(lambda x: f"{x:.4f}")
            ...
            for col in pivot.columns:
                if col == '总计':
                    continue
                col_mask = data_copy['_date_group'] == col
                if value_type == 'count':
                    col_totals[col] = data_copy.loc[col_mask, feature].isna().sum()
                else:
                    col_data = data_copy.loc[col_mask, feature]
