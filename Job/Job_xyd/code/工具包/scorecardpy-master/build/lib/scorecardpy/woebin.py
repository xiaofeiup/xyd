...
def woebin(dt, y, x=None, 
    ...
    var_skip: Name of variables that will skip for binning. Defaults to None. 指定不需要分箱的变量
    breaks_list: List of break points, default is None.  分割点的List。对分箱进行调整的时候用。可以进行自定义分箱
    ...
    stop_limit: Stop binning segmentation when information value 
      ...
      Accepted range: 0-0.5; default is 0.1. 当IV值的增加值小于stop_limit或者卡方值小于qchisq(1-stoplimit, 1)时停止分割
    bin_num_limit: Integer. The maximum number of binning.最大分箱数
    ...
    method: Optimal binning method, it should be "tree" or "chimerge". 
      Default is "tree". 分箱方法，可以有"tree" or "chimerge"
    ignore_const_cols: Logical. Ignore constant columns. Defaults to True.是否忽略常数列
    ...
    check_cate_num: Logical. Check whether the number of unique values in 
      ...
      if there are too many unique categories. Defaults to True.检查分类变量中类别数是否大于50
    replace_blank: Logical. Replace blank values with None. Defaults to True.将空值替换为None
