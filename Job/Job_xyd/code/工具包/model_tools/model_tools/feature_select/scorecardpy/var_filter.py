...
def var_filter(dt, y, x=None, iv_limit=0.02, missing_limit=0.95, 
               ...
               method='merge_small', min_samples=10, smooth_factor=1.0):
    '''
    Variable Filter
    ------
...
#    dt = rep_blank_na(dt)
    ...
    # -iv
    if iv_limit is not None:
        print('[INFO] start to calculate iv ...')
        iv_list = iv(dt, y, x, order=False, method=method, min_samples=min_samples, smooth_factor=smooth_factor)
    else:
        # 如果iv_limit为None，则返回全部特征
        print('[INFO] iv_limit is None, return all features ...')
        # iv_list = pd.DataFrame(columns=['variable', 'info_value'])
        #for fea in x:
        #    iv_list = iv_list.append({'variable': fea, 'info_value': np.nan}, ignore_index=True)
        iv_list_data = []
        for fea in x:
            iv_list_data.append({'variable': fea, 'info_value': np.nan})
        iv_list = pd.DataFrame(iv_list_data)
    # -na percentage
    nan_rate = lambda a: a[a.isnull()].size/a.size
    print('[INFO] start to calculate missing rate ...')
    ...
    print('[INFO] start to calculate identical rate ...')
